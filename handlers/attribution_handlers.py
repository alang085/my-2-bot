...
async def change_orders_attribution(
    ...
    for old_group_id, stats in old_group_stats.items():
        ...
        if stats["valid"]["count"] > 0:
            await update_all_stats(
                "valid", -stats["valid"]["amount"], -stats["valid"]["count"], old_group_id
            )

        # 减少违约订单
        if stats["breach"]["count"] > 0:
            await update_all_stats(
                "breach", -stats["breach"]["amount"], -stats["breach"]["count"], old_group_id
            )
    ...
    if total_valid_count > 0:
        await update_all_stats("valid", total_valid_amount, total_valid_count, new_group_id)

    if total_breach_count > 0:
        await update_all_stats("breach", total_breach_amount, total_breach_count, new_group_id)
