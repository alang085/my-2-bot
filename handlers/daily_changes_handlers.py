...
from decorators import authorized_required, private_chat_only, error_handler
...
async def get_daily_changes(date: str) -> dict:
    ...
    try:
        ...
        # 获取当日本金归还
        principal_records = await db_operations.get_income_records(date, date, type='principal_reduction')
        ...
        completed_orders_amount = sum(float(order.get('amount', 0) or 0) for order in completed_orders)
        
        breach_end_orders_count = len(breach_end_orders)
        breach_end_orders_amount = sum(float(order.get('amount', 0) or 0) for order in breach_end_orders)
        ...
        company_expenses = sum(float(record.get('amount', 0) or 0) for record in expense_records if record.get('type') == 'company')
        other_expenses = sum(float(record.get('amount', 0) or 0) for record in expense_records if record.get('type') == 'other')
