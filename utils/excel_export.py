...
def create_excel_file(
    ...
    # 表头
    headers = ["时间", "订单号", "会员", "归属ID", "订单金额", "状态", "总利息", "利息记录"]
    ...
    for order in orders:
        ...
        order_total_interest = sum(float(interest.get("amount", 0) or 0) for interest in interests)
        total_interest_all += order_total_interest
        ...
        ws_orders.cell(row=row_idx, column=4, value=group_id).border = border
        ws_orders.cell(row=row_idx, column=5, value=float(amount) if amount else 0).border = border
        ws_orders.cell(row=row_idx, column=5).number_format = "#,##0.00"
        ws_orders.cell(row=row_idx, column=5).alignment = right_align
        ws_orders.cell(row=row_idx, column=6, value=state).border = border

        # 总利息列
        ws_orders.cell(row=row_idx, column=7, value=order_total_interest).border = border
        ws_orders.cell(row=row_idx, column=7).number_format = "#,##0.00"
        ws_orders.cell(row=row_idx, column=7).alignment = right_align
        ...
        if interests:
            ...
            ws_orders.cell(row=row_idx, column=8, value=interest_text).border = border
        else:
            ws_orders.cell(row=row_idx, column=8, value="无").border = border
    ...
    if orders:
        ...
        ws_orders.cell(row=row_idx, column=1).alignment = right_align
        ws_orders.cell(row=row_idx, column=5, value=total_amount).number_format = "#,##0.00"
        ws_orders.cell(row=row_idx, column=5).font = Font(bold=True)
        ws_orders.cell(row=row_idx, column=5).alignment = right_align
        ws_orders.cell(row=row_idx, column=5).border = border
        ws_orders.cell(row=row_idx, column=6, value=len(orders)).font = Font(bold=True)
        ws_orders.cell(row=row_idx, column=6).alignment = center_align
        ws_orders.cell(row=row_idx, column=6).border = border
        ws_orders.cell(row=row_idx, column=7, value=total_interest_all).number_format = "#,##0.00"
        ws_orders.cell(row=row_idx, column=7).font = Font(bold=True)
        ws_orders.cell(row=row_idx, column=7).alignment = right_align
        ws_orders.cell(row=row_idx, column=7).border = border
    ...
    ws_orders.column_dimensions["F"].width = 10
    ...
    if completed_orders:
        ...
        for order in completed_orders:
            ...
            order_id = order.get("order_id", "未知")
            amount = order.get("amount", 0)
            updated_at_raw = order.get("updated_at", "") if order.get("updated_at") else "未知"
            updated_at = (
                format_datetime_to_beijing(updated_at_raw) if updated_at_raw != "未知" else "未知"
            )
    ...
    if breach_end_orders:
        ...
        for order in breach_end_orders:
            ...
            ws_breach.cell(row=row_idx, column=3, value=float(amount) if amount else 0).border = (
                border
            )
...
async def export_orders_to_excel(
    ...
    for order in orders:
        ...
        if order_id and order_id in interests_map:
            order_copy["interests"] = interests_map[order_id]
        else:
...
def create_daily_changes_excel_file(
    file_path: str,
    date: str,
    ...
    total_income = total_interest + total_principal
    ...
    summary_data = [
        ...
        [
            "违约完成金额",
            daily_summary.get(
                "breach_end_orders_amount", daily_summary.get("breach_end_amount", 0.0)
            ),
        ],
        ["当日利息", total_interest],
        ["本金归还汇总", total_principal],
        ["收入明细汇总", total_income],
        ["公司开销", daily_summary.get("company_expenses", 0.0)],
        ["其他开销", daily_summary.get("other_expenses", 0.0)],
        ["总开销", total_expenses],
    ...
    if income_records:
        ...
        for record in income_records:
            created_at_raw = record.get("created_at", "") if record.get("created_at") else "未知"
            created_at = (
                format_datetime_to_beijing(created_at_raw) if created_at_raw != "未知" else "未知"
            )
            ...
            ws_income.cell(row=row_idx, column=4, value=float(amount) if amount else 0).border = (
                border
            )
    ...
    if expense_records:
        ...
        for record in expense_records:
            ...
            ws_expense.cell(row=row_idx, column=3, value=float(amount) if amount else 0).border = (
                border
            )
    ...
    for order in orders_data:
        ...
        if interests:
            ...
            for interest in interests:
                ...
                detail_cell = ws_orders.cell(
                    row=row_idx + 1, column=5, value=f"  └─ {interest_date}: {interest_amount:,.2f}"
                )
