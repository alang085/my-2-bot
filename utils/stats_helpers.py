...
async def update_all_stats(
    field: str, amount: float, count: int = 0, group_id: str = None, skip_daily: bool = False
):
    """统一更新所有统计数据（全局、日结、分组）
    ...
    try:
        ...
        if amount != 0:
            try:
                ...
                logger.error(f"❌ 更新全局数据失败 ({global_amount_field}): {e}", exc_info=True)
                raise
        ...
        if count != 0:
            try:
                ...
                logger.error(f"❌ 更新全局计数失败 ({global_count_field}): {e}", exc_info=True)
                raise
        ...
        logger.error(
            f"❌ 更新统计数据失败: field={field}, amount={amount}, count={count}, group_id={group_id}, date={date}, error={e}",
            exc_info=True,
        )
        # 重新抛出异常，让调用者知道更新失败
