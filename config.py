...
def load_config():
    ...
    # 检查是否为生产环境（通过 DATA_DIR 环境变量判断）
    is_production = bool(os.getenv("DATA_DIR"))
    ...
    logger.info(
        f"加载的管理员ID: {admin_ids} (来源: {'环境变量' if is_production or admin_ids_str else 'user_config.py'})"
    )

    if not admin_ids:
        error_msg = "ADMIN_USER_IDS 未设置！\n"
        if is_production:
            error_msg += "请在 Zeabur 环境变量中设置 ADMIN_USER_IDS（多个ID用逗号分隔，无空格）"
        else:
            error_msg += (
                "请选择以下方式之一设置：\n"
