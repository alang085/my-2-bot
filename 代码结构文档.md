...
│   ├── income_handlers.py      # 收入明细处理器
│   ├── order_table_handlers.py # 订单总表处理器
│   ├── daily_summary_handlers.py # 日切数据处理器
│   ├── group_message_handlers.py # 群组消息管理处理器
...
│   └── group_message_callbacks.py # 群组消息回调
│
├── utils/                  # 工具函数
...
│   ├── order_table_helpers.py  # 订单表格生成工具
│   ├── daily_report_generator.py # 日切报表生成器
...
│   ├── db_helpers.py           # 数据库工具
│   ├── print_helpers.py        # 打印辅助工具
│   └── schedule_executor.py    # 定时任务执行器
│
├── scripts/               # 脚本工具
│   ├── __init__.py        # 脚本包初始化
│   ├── init_historical_data.py # 历史数据初始化脚本
│   └── README.md          # 脚本使用说明
│
└── [其他文件]
...
#### `income_handlers.py` - 收入明细
- **仅管理员权限**
- 今日收入明细
- 日期范围查询
- 多维度汇总展示

#### `order_table_handlers.py` - 订单总表
- **仅管理员权限**
- `/ordertable` - 显示订单总表
- 显示所有有效订单及其利息记录
- 支持导出Excel

#### `daily_summary_handlers.py` - 日切数据
- 显示日切数据报表
- 包含新增订单、完结订单、违约完成等统计

#### `group_message_handlers.py` - 群组消息管理
- **仅管理员权限**
- `/groupmsg` - 管理群组消息配置
- `/groupmsg_add` - 添加总群配置
...
#### `order_table_helpers.py` - 订单表格生成
- `generate_order_table()` - 生成订单总表
- `generate_completed_orders_table()` - 生成已完成订单表
- `generate_breach_end_orders_table()` - 生成违约完成订单表

#### `daily_report_generator.py` - 日切报表生成
- `calculate_daily_summary()` - 计算日切数据
- `generate_daily_report()` - 生成完整日切报表

#### `excel_export.py` - Excel导出
- `export_orders_to_excel()` - 导出订单数据到Excel
- 支持多工作表（订单总表、已完成订单、违约完成订单、日切数据）
...
在指定归属的报表中新增"盈余"字段，帮助更好地分析归属ID的盈利情况。
...
- ✅ **仅在归属报表中显示**（`group_id` 不为 None）
- ❌ 全局报表不显示盈余
- ✅ 支持所有时间范围查询（当日、当月、按日期）

### 实现位置

- **计算逻辑**：`handlers/report_handlers.py::generate_report_text()`
- **数据来源**：`db_operations.py::get_stats_by_date_range()`

### 使用示例

```
/report S01  # 查看 S01 归属报表，会显示盈余
```
...
实现总群/总频道的定时消息发送系统，包括：
- **开工信息**：每天11:00自动发送到配置的总群
- **收工信息**：每天23:00自动发送到配置的总群
- **公司公告**：每3小时（可配置）从公告列表中随机选择一条发送
- **欢迎信息**：新成员加入时自动发送（支持变量：{username}, {chat_title}）

### 管理命令

- `/groupmsg` - 管理群组消息配置（仅管理员，私聊）
- `/groupmsg_add <chat_id>` - 添加总群配置
- `/groupmsg_getid` - 获取群组ID（在群组中使用）
- `/announcement` - 管理公司公告（仅管理员，私聊）

### 数据库表

- `group_message_config` - 群组消息配置表
- `company_announcements` - 公司公告表
- `announcement_schedule` - 公告发送计划表
