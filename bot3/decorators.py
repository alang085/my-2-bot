        ...
        try:
            ...
            if update.callback_query:
                ...
                if query.message:
                    ...
                    await asyncio.wait_for(
                        query.answer(short_msg, show_alert=True),
                        timeout=TELEGRAM_API_TIMEOUT,
                    )
            elif update.message:
                await asyncio.wait_for(
            ...
            # 网络超时或连接错误，快速失败，不重试
            logger.warning(
                f"发送错误消息失败（网络超时/连接错误）: {type(timeout_error).__name__}"
            )
            # 不重试，直接退出，避免浪费时间和流量
            break
...
def error_handler(func):
    ...
    async def wrapped(
        ...
        try:
            ...
            error_str = str(e)
            logger.error(
                f"Error in {func.__name__}: {error_type}: {error_str}", exc_info=True
            )
            ...
            try:
                await asyncio.wait_for(
                    ...
                    timeout=TELEGRAM_API_TIMEOUT * (MAX_RETRY_ATTEMPTS + 1)
                    + 2,  # 总超时时间
                )
...
def admin_required(func):
    ...
    async def wrapped(
        ...
        in_admin_list = user_id in ADMIN_IDS if user_id and ADMIN_IDS else False
        logger.info(
            f"admin_required: {func.__name__} - 权限检查结果: {has_permission} "
            f"(用户ID: {user_id}, 在管理员列表中: {in_admin_list})"
        )
        ...
        if not has_permission:
            ...
            if ADMIN_IDS:
                error_msg += (
                    f"Admin ID types: {[type(x).__name__ for x in ADMIN_IDS]}\n"
                )
            error_msg += "\nPlease check:\n"
...
def group_chat_only(func):
    ...
    async def wrapped(
        ...
        if not is_group_chat(update):
            if update.message:
                await update.message.reply_text(
                    "⚠️ This command can only be used in group chat."
                )
            elif update.callback_query:
                await update.callback_query.answer(
