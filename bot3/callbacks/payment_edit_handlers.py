...
# 这些函数在 payment_callbacks_account_edit.py 中定义
# 导入将在运行时动态处理


async def handle_payment_edit_callbacks(
    ...
    if data == "payment_edit_gcash":
        await handle_payment_edit_gcash(update, context, query)
        return True
    elif data == "payment_edit_paymaya":
        await handle_payment_edit_paymaya(update, context, query)
        return True
    elif data.startswith("payment_edit_account_"):
        await handle_payment_edit_account(update, context, query, data)
        return True
    elif data.startswith("payment_edit_info_"):
        await handle_payment_edit_info(update, context, query, data)
        return True
