...
async def handle_expense_company_today(
    ...
    try:
        ...
        records = await get_expense_records_for_callback(
            date, date, expense_type="company"
        )
    except Exception as e:
        logger.error(
            f"handle_expense_company_today: failed to get expense records: {e}",
            exc_info=True,
        )
        try:
    ...
    try:
        ...
        try:
            if query.message:
                ...
                logger.info(
                    "handle_expense_company_today: successfully sent new message"
                )
            else:
...
async def handle_expense_company_month(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    records = await get_expense_records_for_callback(
        start_date, end_date, expense_type="company"
    )
    ...
    keyboard = [
        [InlineKeyboardButton("🔙 返回", callback_data="report_record_company")]
    ]
    try:
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard))
    except Exception as e:
...
async def handle_expense_company_add(
    query, user_id: int, context: ContextTypes.DEFAULT_TYPE
):
    """处理添加公司开销回调"""
    await query.answer()
...
async def handle_expense_other_today(
    query, user_id: int, context: ContextTypes.DEFAULT_TYPE
):
    """处理其他开销今日回调"""
    ...
    try:
        date = get_daily_period_date()
        records = await get_expense_records_for_callback(
            date, date, expense_type="other"
        )
    except Exception as e:
        logger.error(
            f"handle_expense_other_today: failed to get expense records: {e}",
            exc_info=True,
        )
        try:
            await query.answer("❌ 获取开销记录失败", show_alert=True)
...
async def handle_expense_other_month(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    records = await get_expense_records_for_callback(
        start_date, end_date, expense_type="other"
    )
...
async def handle_expense_other_add(
    query, user_id: int, context: ContextTypes.DEFAULT_TYPE
):
    """处理添加其他开销回调"""
    await query.answer()
