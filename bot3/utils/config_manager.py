...
class BotSettings(BaseSettings):
    ...
    def admin_ids(self) -> List[int]:
        ...
        return [
            int(id_str.strip())
            for id_str in self.admin_user_ids.split(",")
            if id_str.strip()
        ]
...
def _load_config_from_env() -> tuple[Optional[str], Optional[str]]:
    """从环境变量加载配置
    ...
    return token, admin_ids_str


def _load_config_from_user_file() -> tuple[Optional[str], Optional[str]]:
    """从user_config.py加载配置
    ...
    try:
        ...
        admin_ids_str = getattr(user_config, "ADMIN_USER_IDS", "")
        return token, admin_ids_str
...
def _load_config_with_fallback() -> tuple[str, str]:
    """加载配置（带回退机制）
    ...
    Raises:
        ValueError: 如果配置未设置
    """
    token, admin_ids_str = _load_config_from_env()
    is_production = bool(os.getenv("DATA_DIR")) or bool(os.getenv("ZEABUR"))

    if not is_production and (not token or not admin_ids_str):
        user_token, user_admin_ids = _load_config_from_user_file()
        token = token or user_token
        admin_ids_str = admin_ids_str or user_admin_ids

    if not token:
        raise ValueError(
            "BOT_TOKEN 未设置！请设置环境变量 BOT_TOKEN 或创建 user_config.py"
        )
...
def _create_settings_from_env() -> BotSettings:
    """从环境变量创建配置实例

    Returns:
        BotSettings实例
    """
    token, admin_ids_str = _load_config_with_fallback()
...
def get_settings() -> BotSettings:
    ...
    if _settings is None:
        try:
            ...
            _settings = _create_settings_from_env()
